...
# the below function are used for hashing passwords and username to improve security
...
# below are functions used  for validate username and passwords    
...
# the bloghandler function contains rendering function to display the content of the page
...
# the register function stores user information from signup form during signup in the user object
          ...
          if not self.user:
               ...
               if (lAuthor == currentUser):
                   self.redirect('/likeUErr')
                    ...
                    post.put()
                    l = LikePost(postid = int(post_id), post = post.key(), parent = blog_key())
                    l.put()
                    self.redirect('/blog')
...
class UpdateComment(BlogHandler):
    def get(self, post_id, comment_id):
        ...
        if (comment.cAuthor == self.user.name):
            self.render("updatecomment.html", subject=post.subject, content=post.content, comment=comment.comment, p = post)
        else:
...
class NewPost(BlogHandler):
     ...
     def post(self):
         ...
         if subject and content:
            p = Post(subject=subject, content=content, author = User.by_name(self.user.name).name, liked_by=[], parent = blog_key())
            p.put()
